...
{aperture:b,exposure:f,"iso speed":h};jQuery(document).ready(function(){for(var a=g.getAllFlickrImageObjects(k),b=0,d=a.length;b<d;b++)(function(){var d=a[b];g.getExif(a[b].id,function(a){var b=a.photo.exif;a="";for(var g=[],f=0,h=b.length;f<h;f++){var j=b[f],e=j.tag.toLowerCase(),k=j.label.toLowerCase(),m=l.indexOf(k);-1!==m&&(j=new p[k](e,j),g[m]=j.getHtml())}if(0<g.length)for(b=0;b<l.length;b++)(f=g[b])?a+=f:(j=new c(e,null),a+=j.getHtml());a&&(e=jQuery(d.node),e=e.width()+parseInt(e.css("border-left-width"),
